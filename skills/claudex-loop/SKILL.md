...
- **Builder:** choose by the kind of difficulty. Clear brief with mostly routine work: `gpt-6-sol`. Hard but clearly stated work, deep debugging (kept whole, not split) or a second attempt after Sol failed: `gpt-6-astra`. Narrow machine-checkable work: `gpt-6-luna`. On the Claude side, Fable only for conception, never for execution of an agreed design; volume-heavy work goes to Opus.
