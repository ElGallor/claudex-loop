...
**Without an explicit choice, do not fall back to the CLI default.** The CLI default (Codex `gpt-6-sol` at low effort since 2026-09-23) fits only clear-brief building and is the wrong answer for review, inspection and hard work. Before the first CLI call, read the section "Select a practical candidate" in the sibling [claudex-route skill](../claudex-route/SKILL.md) (including the allowance check) and pass an explicit `--model` and `--effort` for every role:

- **Reviewer / inspector (judgement work):** Codex side `gpt-6-astra` with `--effort high`; Claude side `claude-fable-5-1`. Never `gpt-6-luna` or `gpt-6-sol` for a judgement review - a weak "APPROVED" is worse than none.
- **Builder:** choose by the kind of difficulty. Clear brief with mostly routine work: `gpt-6-sol` with `--effort low` (`medium` for multi-part documents and plans). Hard but clearly stated work, deep debugging (kept whole, not split) or a second attempt after Sol failed: `gpt-6-astra` with `--effort medium`. Narrow machine-checkable work: `gpt-6-luna` with `--effort high` - lower levels stopped before editing. On the Claude side, Fable only for conception, never for execution of an agreed design; volume-heavy work goes to Opus.
