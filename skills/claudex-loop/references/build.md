...
`workspace-write` grants write access only under the process working directory. A delegated run whose write targets lie outside it -- a configuration file in the user home, a second repository, a tool being relocated -- fails per write with `EPERM`/`EACCES` while the run itself continues to completion. Such a run reports a careful inventory instead of an error, so a report that changed no file is unproven work: verify the target files on disk, and grep the run log for permission errors before accepting it. Choosing a working directory that merely contains the targets is not enough: the user home is not accepted as a writable root, so a run started from it still cannot write `~/.claude` or `~/Documents`. Name every write target outside the workspace explicitly, once per root: `codex exec --skip-git-repo-check -s workspace-write --add-dir <root> ...`. Measured with codex-cli 0.154.0 on Windows -- without `--add-dir` the writes are denied, with it they succeed from an unrelated working directory. The Windows `[windows] sandbox = "unelevated"` mode has further limits that no flag lifts: Node cannot spawn child processes (`spawnSync ... EPERM`, including `cmd.exe`), and subdirectories inside a `tempfile.mkdtemp()` directory fail with `WinError 5`, even with `TEMP` redirected into an added root; Python `subprocess` works. Test suites that spawn Node processes or build temp trees therefore report failures that are sandbox artefacts, not defects. Do not accept such test counts; run those suites outside the sandbox (host or a Claude worker), and the `elevated` mode needs administrator rights. Measured 2026-09-16, codex-cli 0.154.0.
